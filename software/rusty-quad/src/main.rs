...
const IMU_SAMPLE_TIME_SECS: f32 = IMU_SAMPLE_TIME.as_micros() as f32 / 1e6;
...
async fn main(spawner: embassy_executor::Spawner) {
    ...
    let sbus_uart = {
        ...
        UartRx::new(p.UART1, p.PIN_9, p.DMA_CH0, sbus_uart_config)
    };
    ...
    let shared_i2c = {
        ...
        i2c_config.frequency = 400_000; // High speed
        let i2c = I2c::new_async(p.I2C1, p.PIN_11, p.PIN_10, irq, i2c_config);
    ...
    let motors = {
        use embassy_rp::gpio::Pin; // To use PIN.degrade()
        use embassy_rp::pio::PioPeripheral; // To use PIO.split()
